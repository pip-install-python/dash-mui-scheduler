...
3. RETRIES EACH SAMPLE THREE TIMES and records `unreadable` as a state
   ...
   bracket needs its sample — twice out of two on the template — so an
...
# This host, from the ONE source of absolute URLs — never a literal.
try:
    from lib.constants import BASE_URL as _BASE_URL
except Exception:  # pragma: no cover — copied to a runner without the repo
    _BASE_URL = "https://muischeduler.2plot.dev"
DEFAULT_URL = _BASE_URL.rstrip("/") + "/healthz"
SAMPLES = 8
INTERVAL = 45
ATTEMPTS = 3
