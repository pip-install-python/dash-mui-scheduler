...
def test_the_markdown_page_builder_concatenates_rather_than_nesting():
    ...
    # THIS fork's composition sites. pages/home.py is NOT one of them and is
    # deliberately absent: this site has no home.md (DIVERGENCES 4) — the home
    # page is hand-written DMC and runs no parser at all, so naming it here
    # would pin a file that cannot hold the defect. pages/markdown.py is the
    # only place a parsed list meets a children list on this host; it does
    # `layout = section + layout`, the safe concatenation.
