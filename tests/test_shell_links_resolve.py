...
# This fork needed no additions to that set: `/api` is a REGISTERED page here
# (pages/api.py), `/changelog` likewise, and `/webhooks/clerk` is POST-only
# and never linked from the shell. Recorded because an empty delta is a
# statement — the next fork to hit an unserved link should add it here and
# argue for it, not widen the rule with a wildcard.
