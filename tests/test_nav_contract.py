...
def test_api_is_registered_because_this_repo_ships_the_package(app_module):
    """The template's pin inverted, and it has to be: the spec asserts
    API_PACKAGES == [] because the template documents no component package.
    This repo IS the package (DIVERGENCES 1), so the meaningful pin is the
    other branch of contract 7 — declared, therefore registered, therefore
    in the sidebar — and the tables come from the installed wheel's own
    metadata.json, so /api cannot drift from the components shipped."""
    import dash

    from components.navbar import create_content
    from lib.constants import API_PACKAGES

    assert API_PACKAGES == ["dash_mui_scheduler"]
    assert "/api" in [p["path"] for p in dash.page_registry.values()]
    assert "/api" in str(create_content(dash.page_registry.values()))

    from lib import api_reference

    names = [c["name"] for c in api_reference.load_package("dash_mui_scheduler")]
    assert names == ["EventCalendar", "EventCalendarPremium", "EventTimeline",
                     "RadialBarChart", "RadialLineChart"], names
...
def test_the_aside_collapses_on_pages_without_a_toc(app_module):
    ...
    # This fork's docs pages, in place of the template's: every one of the
    # 17 carries a `.. toc::`, so the column is filled on all of them.
    assert has_aside("/quickstart") and has_aside("/radial-lines")
    for path in ("/changelog", "/", "/admin/traffic", "/api"):
        assert not has_aside(path), path
        assert aside_config(path)["collapsed"]["desktop"] is True
    assert aside_config("/quickstart")["collapsed"]["desktop"] is False
