...
did something, and on this host it is the ONLY place that evidence is
visible. So the reads table carries a `verdict` column, rows are grouped
by (path, verdict) rather than by path, and `serve_counts()` keeps served
apart from the rest: a board that adds them together reports enforcement
as traffic, and one that drops the non-served rows reports a policy that
never fired.
"""
